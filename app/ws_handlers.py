...
def _build_system_instruction(session_id: str, orchestrator: LiveOrchestrator, client_id: str = "default") -> str:
    ...
    base = (
        ...
        "- Speak in a natural, conversational tone optimised for audio delivery\n"
        "- Match length to the question: simple factual questions get a short direct answer; "
        "explanations, comparisons, summaries, or step-by-step walkthroughs get as long as needed to be complete and useful\n"
        "- NEVER cut an answer short just to be brief — finish your thought fully\n"
        "- For longer answers, use spoken transitions ('First…', 'Second…', 'Also…', 'Finally…') "
        "so the listener can follow the structure without seeing text\n"
        "- Avoid bullet-point or numbered-list formatting — speak in flowing sentences instead\n\n"
        "Grounding and accuracy:\n"
        "- Ground responses in the uploaded source content whenever relevant\n"
        "- If sources do not contain the answer, say so clearly and offer what you do know\n"
        "- Use natural attribution phrases (e.g. 'According to the document…', 'Based on what you uploaded…')\n\n"
